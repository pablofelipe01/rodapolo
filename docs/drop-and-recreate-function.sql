-- ============================================================================
-- ELIMINAR Y RECREAR FUNCIÓN CREATE_SIMPLE_BOOKING
-- Paso 1: Eliminar función antigua, Paso 2: Crear función nueva
-- ============================================================================

-- 1. Eliminar todas las versiones de la función
DROP FUNCTION IF EXISTS create_simple_booking(UUID, UUID);
DROP FUNCTION IF EXISTS create_simple_booking(UUID, UUID, UUID);

-- 2. Crear la función corregida con solo 2 parámetros
...
BEGIN
  ...
  SELECT p.id INTO v_parental_id
  FROM public.profiles p
  WHERE p.user_id = auth.uid() AND p.role = 'parental';
  ...
  IF NOT EXISTS (
    ...
    WHERE jp.id = p_junior_id AND jp.parental_id = v_parental_id
  ) THEN
    RAISE EXCEPTION 'Junior does not belong to the authenticated parental';
  END IF;
  
  -- Buscar un ticket disponible del parental (con alias explícitos)
...
FROM pg_proc 
WHERE proname = 'create_simple_booking';